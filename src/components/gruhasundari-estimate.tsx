...
const generatePDF = async (
  ...
  container.innerHTML = `
    <div style="border-bottom:2px solid #111;padding-bottom:20px;margin-bottom:20px;">
      <h1 style="font-size:28px;letter-spacing:4px;margin:0;">ANANDS GRUHASUNDARI</h1>
      <p style="margin:4px 0;color:#555;">Ring Road, Vizianagaram</p>
      <p style="margin:0;font-size:12px;color:#888;">Blinds · Curtains · Furnishings</p>
    </div>

    <h2 style="margin-bottom:20px;">Estimate Summary</h2>
  `;
  ...
  windows.forEach((w, i) => {
    ...
    container.innerHTML += `
      <div style="border:1px solid #ddd;padding:16px;margin-bottom:16px;">
        <h3 style="margin:0 0 8px;">${w.label}</h3>
        <p style="margin:0;font-size:13px;">Type: ${w.cat.toUpperCase()}</p>
        <p style="margin:0;font-size:13px;">
          Size: ${c.wFt.toFixed(2)} ft × ${c.hFt.toFixed(2)} ft
        </p>
    `;

    if (w.cat === "blinds" && c.selBlind) {
      container.innerHTML += `
        <p style="margin:6px 0;">Style: ${c.selBlind.name}</p>
        <p style="margin:0;">Area: ${c.sqFtRounded} sq.ft</p>
        <p style="margin:0;">Material: ₹${fmtINR(c.blindBase)}</p>
        <p style="margin:0;">Fixing: ₹${BLIND_FIXING_PER_UNIT}</p>
        <h4 style="margin-top:8px;">Total: ₹${fmtINR(c.blindTotal)}</h4>
      `;
    }

    if (w.cat === "curtains" && c.selCurtain) {
      container.innerHTML += `
        <p style="margin:6px 0;">Style: ${c.selCurtain.name}</p>
        <p style="margin:0;">Panels: ${c.curtainCount}</p>
        <p style="margin:0;">Cloth: ${c.totalClothMeters.toFixed(2)} m</p>
        <p style="margin:0;">Cloth Cost: ₹${fmtINR(c.curtainClothCost)}</p>
        <p style="margin:0;">Stitching: ₹${fmtINR(c.curtainStitch)}</p>
        ${
          c.selTrack
            ? `<p style="margin:0;">Track: ₹${fmtINR(c.trackTotal)}</p>`
            : ""
        }
        <h4 style="margin-top:8px;">Total: ₹${fmtINR(c.curtainGrandTotal)}</h4>
      `;
    }
  ...
  pdf.save("Gruhasundari_Estimate.pdf");
...
export default function GruhasundariEstimate() {
  ...
  const handleQuote = async () => {
    ...
    // 👉 Generate PDF
    await generatePDF(windows, allCalcs, grandTotal);
